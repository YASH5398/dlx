...
const Stat: React.FC<{ label: string; value: React.ReactNode }> = ({ label, value }) => (
  <div className="rounded-xl bg-white/5 ring-1 ring-white/10 p-4">
    ...
    <div className="mt-1 text-xl sm:text-2xl font-semibold text-slate-100">{value}</div>
  </div>
);
    ...
    <div className={`group relative flex flex-col rounded-2xl bg-gradient-to-br from-slate-900 to-slate-800 ring-1 ring-white/10 shadow-xl transition-all duration-300 ${unlocked ? 'hover:shadow-emerald-500/20 hover:ring-emerald-400/30' : 'hover:shadow-2xl hover:ring-violet-500/30'}`}> 
      <div className="p-4 flex items-start gap-4">
        <img
          src={badge.image}
          alt={badge.name}
          onError={(e) => { e.currentTarget.src = BULLET_BIKE_URL; e.currentTarget.onerror = null; }}
          className="size-16 sm:size-20 rounded-lg ring-1 ring-white/15 object-cover"
        />
        <div className="flex-1">
          <div className="flex items-center gap-2">
            <h3 className="text-base sm:text-lg font-semibold text-slate-100">{badge.name}</h3>
            <span
              className={`text-xs px-2 py-1 rounded-full ring-1 ${unlocked ? 'bg-emerald-500/15 text-emerald-300 ring-emerald-400/30' : 'bg-rose-500/10 text-rose-300 ring-rose-400/30'}`}
            >
      ...
      <div className="px-4 pb-4 space-y-4">
        ...
        <div className="flex gap-2 pt-2">
          <button
            ...
            className="flex-1 h-11 inline-flex items-center justify-center gap-2 rounded-lg bg-white/5 ring-1 ring-white/10 text-slate-200 hover:bg-white/10 hover:ring-violet-400/30 transition-colors px-3"
          >
          ...
          <button
            ...
            aria-expanded={open}
            aria-controls={`badge-${badge.name}-details`}
            className="h-11 inline-flex items-center justify-center gap-2 rounded-lg bg-gradient-to-r from-indigo-600 to-fuchsia-600 text-white hover:from-indigo-500 hover:to-fuchsia-500 transition-colors px-3"
          >
        ...
        <div
          id={`badge-${badge.name}-details`}
          className={`grid grid-cols-1 sm:grid-cols-2 gap-3 rounded-lg bg-white/5 ring-1 ring-white/10 p-3 transition-all duration-300 overflow-hidden ${open ? 'opacity-100 max-h-96' : 'opacity-0 max-h-0'}`}
        >
          <div className="text-xs sm:text-sm text-slate-300">
            <div className="font-medium text-slate-200 mb-1">Referrals</div>
            <div>Progress: {stats.referrals}/{badge.referralsRequired} ({referralPct}%)</div>
            <div>Remaining: {Math.max(0, badge.referralsRequired - stats.referrals)}</div>
          </div>
          <div className="text-xs sm:text-sm text-slate-300">
            <div className="font-medium text-slate-200 mb-1">Volume</div>
            <div>Progress: {formatVolume(stats.volume)}/{formatVolume(badge.volumeRequired)} ({volumePct}%)</div>
            <div>Remaining: {formatVolume(Math.max(0, badge.volumeRequired - stats.volume))}</div>
          </div>
        </div>
      </div>
    </div>
