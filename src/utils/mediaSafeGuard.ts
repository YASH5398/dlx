...
const lastPlayTime = new WeakMap<HTMLMediaElement, number>();

function patchMediaPlayback() {
  if (typeof (window as any).HTMLMediaElement === 'undefined') return;
  const proto = (window as any).HTMLMediaElement.prototype as HTMLMediaElement;
  ...
  proto.play = function(this: HTMLMediaElement): Promise<void> {
    ...
    try {
      const p = origPlay.apply(el) as Promise<void>;
