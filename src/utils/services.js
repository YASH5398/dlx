...
function getExtraFields(serviceName) {
...
function appendExtraStep(base, serviceName) {
    const names = new Set();
    base.forEach((s) => s.fields.forEach((f) => names.add(f.name)));
    const extras = getExtraFields(serviceName).filter((f) => !names.has(f.name));
    if (!extras.length)
        return base;
    const extraStep = { title: 'Additional Requirements', fields: extras };
    return [...base, extraStep];
}
export async function restoreDefaultServiceForms(defaults) {
...
export async function getServiceFormConfig(serviceName) {
    const snap = await get(ref(db, `serviceForms/${serviceName}`));
    const val = snap.val();
    const steps = val || null;
    // Fall back to predefined defaults when not found (handled by admin setup)
    if (!steps)
        return null;
    return steps;
}
export async function setServiceFormConfig(serviceName, steps) {
    await set(ref(db, `serviceForms/${serviceName}`), steps);
}
export function subscribeServiceFormConfig(serviceName, cb) {
    const r = ref(db, `serviceForms/${serviceName}`);
    const handler = (snap) => {
        const val = snap.val();
        const steps = val ? val : null;
        cb(steps);
    };
    onValue(r, handler);
...
export function subscribeServices(cb) {
