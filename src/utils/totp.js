...
async function hmacSha1(key, msg) {
    const keyView = new Uint8Array(key.buffer);
    const msgView = new Uint8Array(msg.buffer);
    const cryptoKey = await crypto.subtle.importKey('raw', keyView, { name: 'HMAC', hash: 'SHA-1' }, false, ['sign']);
...
export async function generateTOTP(secretBase32, step = 30, digits = 6, forTime) {
...
export async function verifyTOTP(secretBase32, code, window = 1, step = 30, digits = 6) {
    const now = Date.now();
    const offsets = [];
...
export function qrCodeUrlFromOtpauth(otpauth, size = 200) {
